...
func GetStatus(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		...
		"access_code_enabled":         config.UserAccessCode != "",
		},
	})
	return
}
...
func SendPasswordResetEmail(c *gin.Context) {
	...
	content := message.EmailTemplate(
		subject,
		fmt.Sprintf(`
			<p>您好！您正在进行密码重置，请复制以下 Token：</p>
			<p style="font-size: 24px; font-weight: bold; color: #333; background-color: #f8f8f8; padding: 12px; text-align: center; border-radius: 4px; letter-spacing: 1px; word-break: break-all; user-select: all; -webkit-user-select: all;">%s</p>
			<p>或点击下面的按钮进行密码重置：</p>
		...
		`, code, link, link, common.VerificationValidMinutes),
	)
	err := message.SendEmail(subject, email, content)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
